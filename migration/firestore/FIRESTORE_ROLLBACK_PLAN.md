...
Re-verified 2026-09-17 (second readiness pass) and unchanged: cutover is still NOT_STARTED, production customer
writes remain 0, and the rollback rehearsal evidence is still emulator-scoped
(`firestore-spark-client-smoke.json`, `target: "EMULATOR"`). The production client-smoke gate now reads a separate
artifact, so that emulator rehearsal can no longer be mistaken for a real-project rollback rehearsal.

...
const indexes=indexReadiness(required,inventory.evidence.indexes).map((entry,i)=>{
  ...
  return {...entry,id:`required-${i+1}`,command:`gcloud ${args.map(quote).join(' ')}`};
});
const report={generatedAt:new Date().toISOString(),project:'mydesckpro',database:'default',mode:'plan',productionMutations:0,
  required:indexes.length,ready:indexes.filter(i=>i.state==='READY').length,creating:indexes.filter(i=>i.state==='CREATING').length,
  ...
  note:'Create only MISSING entries. CREATING is not READY. API/permission failures are ERROR, not empty lists. Field overrides in the broader config are not part of these three commands.'};

...
let log = '# Real Identifiers & Runner Readiness Commit Log\n\n';
...
exec('2. Stage workflow & script files', 'git add -A .github/workflows/ scripts/ docs/ package.json package-lock.json');

// 4. Commit
exec('3. Commit real identifiers & runner readiness changes', 'git commit -m "fix: require real staging identifiers and runner readiness"');
...
console.log('Committed and pushed real identifiers and runner readiness changes. Output saved to ' + outputFile);

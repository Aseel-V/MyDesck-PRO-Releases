...
let log = '# Observed Harness Commit Execution Log\n\n';
...
exec('1. Stage all harness & workflow files', 'git add .github/workflows/staging-pipeline.yml .github/workflows/build-release.yml package.json package-lock.json scripts/run-windows-upgrade-harness.mjs scripts/verify-desktop-upgrade-physical.mjs scripts/aggregate-staging-evidence.mjs scripts/verify-staging-database.mjs scripts/test-staging-e2e.mjs scripts/verify-desktop-release-assets.mjs scripts/verify-desktop-updater-metadata.mjs scripts/test-workflow-negative-rules.mjs docs/RELEASE_SYSTEM_ARCHITECTURE.md docs/STAGING_PROCESS.md');

exec('2. Commit changes', 'git commit -m "fix: require observed Windows upgrade evidence"');

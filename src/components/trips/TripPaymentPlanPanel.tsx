...
export function TripPaymentPlanPanel({ trip, onClose }: Props) {
  ...
  const [firstDate, setFirstDate] = useState(getTravelBusinessDate());
  const [editing, setEditing] = useState<string | null>(null);
  const [paidAmount, setPaidAmount] = useState('');
  const [actionDate, setActionDate] = useState(getTravelBusinessDate());
      ...
      <main className="space-y-5 p-4">
        {query.data?.plan ? <>
          ...
          {summary && <div className="grid grid-cols-2 gap-2 md:grid-cols-6">{[
            ['total',summary.expectedMinor],['paidBySchedule',summary.paidMinor],['remaining',summary.remainingMinor],['manualVisaReceived',summary.manualReceivedMinor],

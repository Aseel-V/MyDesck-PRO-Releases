-- Forward-only migration to fix PostgreSQL 42P10: there is no unique or exclusion constraint matching the ON CONFLICT specification
-- Ensures unique constraint on public.trip_payment_plans(trip_id) and refactors payment plan upsert logic in save_trip_transaction to be 100% resilient.

-- 1. Ensure UNIQUE constraint exists on public.trip_payment_plans(trip_id)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = 'public.trip_payment_plans'::regclass
      AND contype = 'u'
      AND conname = 'trip_payment_plans_trip_id_key'
  ) THEN
    -- Deduplicate any existing historical payment plans per trip_id keeping the latest record
    DELETE FROM public.trip_payment_plans a
    USING public.trip_payment_plans b
    WHERE a.trip_id = b.trip_id AND a.created_at < b.created_at;

    -- Drop partial index if exists to avoid conflict
    DROP INDEX IF EXISTS public.trip_payment_plans_one_active_idx;

    -- Add unique constraint
    ALTER TABLE public.trip_payment_plans
      ADD CONSTRAINT trip_payment_plans_trip_id_key UNIQUE (trip_id);
  END IF;
EXCEPTION WHEN OTHERS THEN
  -- Ignore if constraint or table modification already completed
  NULL;
END $$;

-- 2. Update save_trip_transaction with safe payment plan lookup and upsert
...
BEGIN
  ...
  -- 3. Perform Trip Upsert (excluding database-managed columns: profit, profit_percentage, amount_due, search_document, created_at)
  IF v_is_edit THEN
  ...
  IF p_payment_plan IS NOT NULL AND (p_payment_plan->>'method') IS NOT NULL THEN
    ...
    IF v_payment_method IN ('card', 'mixed') THEN
      ...
      -- Upsert Payment Plan using explicit lookup (100% 42P10 proof)

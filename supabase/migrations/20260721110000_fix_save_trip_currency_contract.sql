-- Forward-only migration to fix trip currency saving contract in public.save_trip_transaction
-- Ensures currency, exchange_rate, sale_currency, sale_original_amount, wholesale_currency, and wholesale_original_amount
-- are explicitly saved on INSERT and UPDATE to prevent default 'USD' overriding new ILS trips.

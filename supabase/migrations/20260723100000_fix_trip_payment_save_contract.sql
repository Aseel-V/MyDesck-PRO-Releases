-- Forward-only correction for the native Travel Mode payment save contract.
-- The previous RPC revision used non-existent trip_installments columns/status values.
-- This keeps the RPC signature, ownership checks, currency handling, and idempotency behavior intact.
...
COMMENT ON FUNCTION public.save_trip_transaction(jsonb, jsonb, uuid) IS
  'Atomically saves an owned trip and native cash/card/mixed plan using the canonical installment schema.';
